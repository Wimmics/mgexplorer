...
export class MgeAnnotation {
    ...
    constructor() {
        ...
        console.log("dashboard = ", this._dashboard)
    }
        ...
        select(this.element.querySelector("#saveAnnotationbtn")).on("click", async () => {
            ...
            if (cat == 'dashboard' || cat == 'object') {
                ...
                if (cat == 'object') {
                    ...
                    this._dashboard._addLinkAnnotation(parents, this._view)
                    .then(async (links) => {
                        for (let i = 0; i < links.lines.length; i++) {
                            let line = links.lines[i]
                            await this._dashboard.addChart(parents[i].idView, {
                                id: that._view.idView, 
                                title: this.element.id, 
                                typeChart: that._view.typeVis, 
                                hidden: false, 
                                x: that._view.x, 
                                y: that._view.y, 
                                view: that._view, 
            ...
            // this.saveAnnotationContent(state.formData); // TODO: review the saving process (disabled for now)
            this.disableForm();
