...
export class MgeHistory {
    ...
    async addHistoryTreeChart(idDiv, divTag) {
        ...
        this.model.when(["data"], () => {
            ...
            let height = this._grpHistory.node().getBBox().height
            if (height && height > this.height )
                _svg.attr('height', height + 50)
    ...
    */
    ...
    saveHistoryData() {
        select(this.element.querySelector("#saveHistorybtn")).on("click", () => {
            ...
            this.download(CircularJSON.stringify(this._htdata, undefined, 4), `mgexplorer_history_${this._htdata.time}.json`)
        })
            ...
            <Host>
                <div class="history-tree">
                    <div class="history-buttons" style={{ "position": "absolute", "bottom": "10px", "width": "100%", "text-align": "right" }}>
                        <button type='button' class="btn btn-outline-primary" id="saveHistorybtn">Download</button>
                    </div>
                </div>
            </Host>
        );
